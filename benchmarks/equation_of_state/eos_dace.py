...
@dc.program
def gsw_dHdT(sa: dc.float64[S1, S1, S2],
             ct: dc.float64[S1, S1, S2],
             p: dc.float64[1, 1, S2]):
    """
...
cpu_sdfg = gsw_dHdT.to_sdfg(strict=False)
import copy
...
def run(sa, ct, p, device='cpu'):
    ...
    if device == 'cpu':
        return cpu_exec(sa=sa, ct=ct, p=p, S1=sa.shape[0], S2=sa.shape[-1])
    elif device == 'gpu':
        return gpu_exec(sa=sa, ct=ct, p=p, S1=sa.shape[0], S2=sa.shape[-1])